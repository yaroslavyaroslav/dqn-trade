...
class DQN(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f'x_view.shape: {x.view(x.size(0), 512).shape}')
        # print(f'x_view: {x.view(x.size(0), -1)}')
        result = self.head(x.view(x.size(0), -1))
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            some_state = policy_net(state)
            print(f'some_state: {policy_net(state)}')
            print(f'some_state.max(1)[1]: {some_state.max(1)[1]}')
            print(f'some_state.max(1)[1].view(1, 1): {some_state.max(1)[1].view(1, 1)}')
            return some_state.max(1)[1].view(1, 1)
    else:
