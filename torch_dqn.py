...
class DQN(nn.Module):
    ...
    def __init__(self, input_size, outputs):
        ...
        self.conv1 = nn.Linear(input_size, 512)
    ...
    def forward(self, x):
        ...
        # x = x.double64()
        # print(x.dtype)
        if type(x) is np.ndarray: x = torch.from_numpy(x)
        x = x.float().to(device)
        x = torch.flatten(x)
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            print(f'some_test: {policy_net(state)}') #.view(1, 1))

            return policy_net(state).max(0)[0].view(1, 1)
    else:
...
def optimize_model():
    ...
    # print(f'batch.next_state: {type(batch.next_state[0])}')
